// stasm_main.cpp: command line utility to run Stasm on a set of images
//
// Copyright (C) 2005-2013, Stephen Milborrow

#include <iostream>
#include <stdio.h>

#include <opencv2/highgui/highgui.hpp>
#include <opencv2/imgproc/imgproc.hpp>

#include "stasm.h"
#include "opencv/highgui.h"
#include "appmisc.h"

using namespace std;
using namespace cv;

cv::String MyWin_Name = "stasm_cam";

namespace stasm
{
static bool writeimgs_g = true;               // -i flag
static bool multiface_g;                      // -m flag
static int  minwidth_g = 25;                  // -s flag
static int  nlandmarks_g = stasm_NLANDMARKS;  // -n flag
static bool csv_g;                            // -c flag

//-----------------------------------------------------------------------------

static void GetOptions(
    int&          argc, // in
    const char**& argv) // io
{
    static const char* const usage =
        "stasm [FLAGS] IMAGE [IMAGE2 ...]\n"
        "\n"
        "Landmark results are saved in stasm.log\n"
        "\n"
        "Example: stasm face.jpg\n"
        "\n"
        "Flags:\n"
        "    -m     allow multiple faces per image (default is just the largest)\n"
        "    -s     detect small faces (min width 10%% of image width, the default is 25%%)\n"
        "    -n N   save as 77, 76, 68, 22, 20, or 17 point shape\n"
        "    -c     save landmarks as CSVs (default saves in shapefile format)\n"
        "    -i     do not write landmarked images (faster)\n"
        "    -d     enable debug prints\n"
        "    -?     help\n"
        "\n"
        "Stasm version %s    www.milbo.users.sonic.net/stasm\n"; // %s is stasm_VERSION

    if (argc < 2)
        Err("No image.  Use stasm -? for help.");
    while (--argc > 0 && (*++argv)[0] == '-')
    {
        if ((*argv + 1)[1])
            Err("Invalid argument -%s (there must be a space after -%c).  "
                "Use stasm -? for help.",
                *argv + 1, (*argv + 1)[0]);
        switch ((*argv + 1)[0])
        {
        case 'c':
            csv_g = true;
            break;
        case 'd':
            trace_g = true; // trace_g defined in print.cpp
            break;
        case 'i':
            writeimgs_g = false;
            break;
        case 'm':
            multiface_g = true;
            break;
        case 's':
            minwidth_g = 10;
            break;
        case 'n':
            if (argc < 3)
                Err("-n argument must be followed by NLANDMARKS.  For example -n 68");
            argc--;
            argv++;
            nlandmarks_g = -1;
            if (1 != sscanf(*argv, "%d", &nlandmarks_g) || nlandmarks_g < 1)
                Err("-n argument must be followed by NLANDMARKS.  For example -n 68");
            // validity of nlandmarks_g will be checked later after call to ConvertShape
            break;
        case '?':
            printf(usage, stasm_VERSION);
            exit(1);
        default:
            Err("Invalid argument -%s.  Use stasm -? for help.", *argv + 1);
            break;
        }
    }
    if (argc < 1)
        Err("No g.  Use stasm -? for help.");
}

static void ProcessFace(
    CImage&      cimg,      // io: color version of image
    const float* landmarks, // in
    int          nfaces,    // in
    const char*  imgpath)   // in
{
    Shape shape(LandmarksAsShape(landmarks));
    Shape converted_shape(ConvertShape(shape, nlandmarks_g));
    if (converted_shape.rows == 0)
    {
        static int printed;
        PrintOnce(printed, "\nCannot convert a %d point shape to a %d point shape\n",
                  shape.rows, nlandmarks_g);
    }
    else
        shape = converted_shape;
    if (writeimgs_g)
        DrawShape(cimg, shape);
    if (multiface_g)
    {
        logprintf("face %d\n", nfaces);
        double xmin, xmax, ymin, ymax; // needed to position face nbr
        ShapeMinMax(xmin, xmax, ymin, ymax, shape);
        ImgPrintf(cimg,
                  (xmin + xmax)/2, ymin - (ymax-ymin)/50., C_RED, 1,
                 "%d", nfaces);
    }

    if (csv_g)
        LogShapeAsCsv(shape, imgpath);
    else
        LogShape(shape, imgpath);
}

static void ProcessImg(
    const char* imgpath) // in
{
    Image img(cv::imread(imgpath, CV_LOAD_IMAGE_GRAYSCALE));
    if (!img.data)
        Err("Cannot load %s", imgpath);
    if (!stasm_open_image((const char*)img.data, img.cols, img.rows, imgpath,
                          multiface_g, minwidth_g))
        Err("stasm_open_image failed:  %s", stasm_lasterr());

    CImage cimg;     // color version of image
    if (writeimgs_g) // actually need the color image?
        cvtColor(img, cimg, CV_GRAY2BGR);
    int nfaces = 0;
    while (1)
    {
        if (trace_g && nfaces > 0 && multiface_g)
            stasm_printf("\n%d: ", nfaces);

        int foundface;
        float landmarks[2 * stasm_NLANDMARKS]; // x,y coords
        if (!stasm_search_auto(&foundface, landmarks))
            Err("stasm_search_auto failed: %s", stasm_lasterr());

        if (!foundface)
            break; // note break

        ProcessFace(cimg, landmarks, nfaces, Base(imgpath));
        nfaces++;
    }
    if (trace_g)
        lprintf("\n");
    if (writeimgs_g && nfaces)
    {
        // write as a bmp not as a jpg because don't want blurred shape lines
        char newpath[SLEN]; sprintf(newpath, "%s_stasm.bmp", Base(imgpath));
        lprintf("%s ", newpath);
        if (!cv::imwrite(newpath, cimg))
            Err("Cannot write %s", newpath);
    }
    lprintf("%d face%s\n", nfaces, plural(nfaces));
}

static void main1(int argc, const char** argv)
{
    GetOptions(argc, argv);
    OpenLogFile();

    // argc is now the number of images and argv is the image filenames

    const bool old_trace = trace_g;
    if (!stasm_init("../data", 1 /*trace*/))
        Err("stasm_init failed %s", stasm_lasterr());
    trace_g = old_trace;

    const int ndigits = NumDigits(argc); // for aligning prints

    for (int i_img = 0; i_img < argc; i_img++)
    {
        const char* const imgpath = argv[i_img];
        if (argc > 1)
            lprintf("%*d ", ndigits, i_img);
        lprintf("%s: ", imgpath);
        ProcessImg(imgpath);
    }
}

static void ProcessCamImg(
    Mat& frame) // in
{
//    Image img(cv::imread(imgpath, CV_LOAD_IMAGE_GRAYSCALE));
	Image img(frame);
    if (!img.data){
        Err("Cannot load frame");
	}
    if (!stasm_open_image((const char*)img.data, img.cols, img.rows,
    			"/home/thomas/build/stasm/stasms.git/build",
                          multiface_g, minwidth_g)){
        Err("stasm_open_image failed:  %s", stasm_lasterr());
	}
    CImage cimg;     // color version of image

    if (writeimgs_g) // actually need the color image?
        cvtColor(img, cimg, CV_GRAY2BGR);
    int nfaces = 0;
    while (1)
    {
        if (trace_g && nfaces > 0 && multiface_g)
            stasm_printf("\n%d: ", nfaces);

        int foundface;
        float landmarks[2 * stasm_NLANDMARKS]; // x,y coords
        if (!stasm_search_auto(&foundface, landmarks))
            Err("stasm_search_auto failed: %s", stasm_lasterr());

        if (!foundface)
            break; // note break

		ProcessFace(cimg, landmarks, nfaces, Base("/home/thomas/build/stasm/stasms.git/build"));
		cv::imshow(MyWin_Name, cimg);
        nfaces++;
    }
    if (trace_g)
        lprintf("\n");
    if (writeimgs_g && nfaces)
    {
        // write as a bmp not as a jpg because don't want blurred shape lines
        char newpath[SLEN]; sprintf(newpath, "%s_stasm.bmp", Base("/home/thomas/build/stasm/stasms.git/build"));
        lprintf("%s ", newpath);
        if (!cv::imwrite(newpath, cimg))
            Err("Cannot write %s", newpath);
    }
    lprintf("%d face%s\n", nfaces, plural(nfaces));
}

static void main2(int argc, const char** argv)
{
	cv::VideoCapture vc;
	int index=-1;

	if(argc>1){
		for(int i=1;(i< argc) && (argv[i][0]=='-') ;i++){
			switch(argv[i][1]){
			case 'C':
				index = std::atoi( argv[i]+2 );
				if(!vc.open(index)){
					std::cout << " open cam device index:"<< index <<"failed." <<std::endl;
					exit(-1);
				}
				//cvCamCap(vc);
				//nSourceType=sCamera;
				break;
			case 'F':
				if(!vc.open(argv[i]+2)){
					std::cout << " open media file"<< argv[i]+2 <<" failed." <<std::endl;
					exit(-1);
				}
				//nSourceType=sVideoFile;
				break;
			default:
				break;
			}
		}
	}else{
		std::cout <<"-Cnnn: index of camera ,-1(auto),0,1,2 camera index" << std::endl;
		std::cout <<"-Fpath: a static piture or motion picture file" << std::endl;
		vc.open(-1);
		exit(-1);
	}

    const bool old_trace = trace_g;
    if (!stasm_init("../data", 1 /*trace*/))
        Err("stasm_init failed %s", stasm_lasterr());
    trace_g = old_trace;

  	//-- 2. Read the video stream
	cv::namedWindow(MyWin_Name);
	if(vc.isOpened()){
		for(;;){
			cv::Mat colorMat, greyMat;
			vc >> colorMat; 	//get one frame
		  	if(!colorMat.empty() ){
				cv::cvtColor(colorMat, greyMat, CV_BGR2GRAY);
			  	ProcessCamImg(greyMat);
//				cv::imshow(MyWin_Name, greyMat);
		  	}
		  	int c = waitKey(5);
		  	if( c == 27 || c == 'q' ) break;
		}
	}
}

} // namespace stasm



// This application calls Stasm's internal routines.  Thus we need to catch a
// potential throw from Stasm's error handlers.  Hence the try/catch code below.

int main(int argc, const char** argv)
{
    stasm::CatchOpenCvErrs();
    try
    {
        //stasm::main1(argc, argv);
        stasm::main2(argc,argv);
    }
    catch(...)
    {
        // a call was made to Err or a CV_Assert failed
        printf("\n%s\n", stasm_lasterr());
        exit(1);
    }
    return 0;       // success
}
